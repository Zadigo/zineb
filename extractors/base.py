...
class HtmlTag(BaseTag):
    ...
    def attrs(self):
        ...
        for attr, value in self._attrs:
            items.setdefault(attr, value)
        return items
    
    @property
    def built_attrs(self):
        items = ''
        for key, value in self.attrs.items():
            items = items + f'{key}="{value}"'
        return items
    
    def add_data(self, data):
        self._inner_data.append(data)
    
    
class Link(HtmlTag):
    def __repr__(self):
        return f"{self.__class__.__name__}({self.built_attrs})"
...
class HtmlAlgorithm(HTMLParser):
    def __init__(self, extractor, tag_name, **kwargs):
    ...
    def handle_starttag(self, tag, attrs):   
        self.extractor.start_tag(tag, attrs)
                
    def handle_data(self, data):
        self.extractor.inner_data(data)
...
class ExtractorMixin:
    ...
    def resolve(self, html):
        algorithm = HtmlAlgorithm(self, self.tag_name)
        algorithm.feed(html)
...
class LinkExtractor(ExtractorMixin):
    tag_name = 'a'
    ...
    def start_tag(self, tag, attrs):
        if tag == 'a':
            link = Link(tag, attrs)
            self._current_tag = link
            self.container.append(link)
        print(tag)
            
    def inner_data(self, data):
        if self._current_tag is not None:
            self._current_tag.add_data(self.transform_data(data))
