...
class FileSystemStorage(BaseStorage):
    """A storage for managing files locally
    in a scrapping project. It is automatically
    injected in the current's spider class

    >>> class MySpider(Spider):
            def start(self, **kwargs):
                my_file = self.storage.get_file("my_file.txt")
    """
...
class AWSFileSystemStorage(BaseStorage):
    """A storage for managing files in AWS
    in a scrapping project. It is automatically
    injected in the current's spider class

    >>> class MySpider(Spider):
            def start(self, **kwargs):
                self.storage.get_file("my_file.txt")
    """
